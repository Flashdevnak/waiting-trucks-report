...
function patchFreshnessTruth(source) {
  let output = String(source || "");
  if (output.includes(FRESHNESS_MARKER)) return output;
  ...
  el("source-sync").textContent = state.syncError
    ...
    `// ${FRESHNESS_MARKER}: source-data age is not session health. A HUB may have
// no business changes for minutes while the accepted realtime transport remains
// healthy. Use explicit source status + the latest received snapshot instead.
function renderFreshness() {
  const synced = parseDate(state.lastSync);
  ...
  el("source-sync").textContent = synced
    ...
    "separate source timestamp from session/realtime health",
  );
...
export function patchMsLiveResilienceFrontend(source) {
  ...
  // The resilience base may already be integrated into canonical ms.js. Keep
  // that base idempotent, but always apply newer truth fixes owned by this same
  // module so a staged DEV deploy cannot regress on older source semantics.
