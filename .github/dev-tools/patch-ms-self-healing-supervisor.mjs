...
export function patchMsSelfHealingSupervisor(source) {
  ...
  output = replaceOnce(
    ...
    `// ${MARKER}: reuse the existing per-HUB refresh as the health probe.\n// Healthy operation adds zero Turso reads/writes and zero upstream calls.\nconst MS_REPAIR_TRANSIENT_BACKOFF_MS = [60_000, 2 * 60_000, 5 * 60_000, 10 * 60_000];\nconst MS_REPAIR_SESSION_COOLDOWN_MS = 60 * 60_000;\nconst MS_REPAIR_CONCURRENCY = 4;\nconst MS_REPAIR_POLICY_VERSION = 6;\n\nasync function mapMsRepairLimit(items, limit, task) {\n  const result = new Array(items.length);\n  let cursor = 0;\n  async function worker() {\n    while (cursor < items.length) {\n      const index = cursor++;\n      result[index] = await task(items[index], index);\n    }\n  }\n  await Promise.all(Array.from({ length: Math.min(Math.max(1, limit), items.length || 1) }, worker));\n  return result;\n}\n\nasync function notifyMsCredentialAdopted(env, branch) {\n  if (!env.MS_REFRESH_COORDINATOR || !branch) return;\n  try {\n    const id = env.MS_REFRESH_COORDINATOR.idFromName(msCoordinatorIdentity(branch));\n    const stub = env.MS_REFRESH_COORDINATOR.get(id);\n    const url = new URL("https://ms-refresh.internal/credential-adopted");\n    url.searchParams.set("branch", branch);\n    const response = await stub.fetch(new Request(url));\n    if (!response.ok)\n      console.error(JSON.stringify({ event: "ms_credential_adoption_reset_failed", hub: branch, status: response.status }));\n  } catch (error) {\n    console.error(JSON.stringify({ event: "ms_credential_adoption_reset_failed", hub: branch, message: error?.message || String(error) }));\n  }\n}\n\nexport async function runMsScheduledRefresh(env) {`,
  ...
  output = replaceOnce(
    ...
    `  await audit(\n    env,\n    "SAVE_MS_CONNECTION",\n    hub,\n    \`ทดสอบสำเร็จ \${test.total} รายการ\`,\n    updatedBy,\n  );\n  // Fresh HAR credentials are authoritative. Clear any cooldown/401 state from the old session immediately.\n  await notifyMsCredentialAdopted(env, hub);\n  return { hub, total: test.total, updatedAt: now };`,
    "fresh HAR bypasses stale repair state",
  );
