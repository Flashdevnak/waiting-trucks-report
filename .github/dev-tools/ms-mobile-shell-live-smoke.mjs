...
async function probePnoModal(cdp,sessionId,width,label){
  const expression = '(async()=>{'+
    ...
    'try{'+
      ...
      'const truthRow={id:\'truth-row\',hub:\'NE1\',attendanceType:\'ปลายทาง\',proofId:\'TRUTH-BC\',pnoState:\'OK\',pnoEnabled:true,pnoSourceDay:\'2026-09-19\',pnoCanReport:false,expectedParcels:355,enteredParcels:253,pendingParcels:102};const ownHubCurrent=Array.from({length:113},(_,i)=>({pno:\'OWN-BAG-\'+i,backingNo:\'BAG-\'+Math.floor(i/40),lastAction:i<96?\'สแกนเข้าคลัง\':i<112?\'แบ็กกิ้ง\':\'ปริ้นท์ใบลาเบล\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'total\'}));const alreadyOwnHub=ownHubCurrent.slice(0,12);const alreadyNonBag=Array.from({length:241},(_,i)=>({pno:\'ALREADY-NONBAG-\'+i,backingNo:\'\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'already\'}));const alreadyCurrent=[...alreadyNonBag,...alreadyOwnHub];const linkedCurrent={pno:\'CURRENT-LINK-0\',backingNo:\'\',lastAction:\'รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'total\'};const totalCurrent=[...alreadyNonBag,...ownHubCurrent,linkedCurrent];const visualBrowserPnoPage=browserPnoPage;browserPnoPage=async (_row,type,page)=>{const src=type===\'total\'?totalCurrent:type===\'already\'?alreadyCurrent:[];const pageSize=200,pages=Math.max(1,Math.ceil(src.length/pageSize)),start=(Math.max(1,page)-1)*pageSize;return {total:src.length,pageSize,pages,page,parcels:src.slice(start,start+pageSize)}};const truthCurrentKey=pnoOperationalKey(truthRow);pnoOperationalTruthCache.delete(truthCurrentKey);pnoOperationalEnteredRowsCache.delete(truthCurrentKey);const truthCurrent=await pnoOperationalResolve(truthRow);browserPnoPage=visualBrowserPnoPage;const isolationTotal=[...Array.from({length:5},(_,i)=>({pno:\'ISO-A-\'+i,lastAction:\'ดำเนินการปกติ\',targetHub:\'02 NE1_HUB-นครราชสีมา\'})),{pno:\'ISO-N1-SCAN\',backingNo:\'BN1\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\'},{pno:\'ISO-N1-BAG\',backingNo:\'BN1\',lastAction:\'แบ็กกิ้ง\',targetHub:\'02 NE1_HUB-นครราชสีมา\'},{pno:\'ISO-N1-LABEL\',backingNo:\'BN1\',lastAction:\'ปริ้นท์ใบลาเบล\',targetHub:\'02 NE1_HUB-นครราชสีมา\'},{pno:\'ISO-N6-SCAN\',backingNo:\'BN6\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'},{pno:\'ISO-N6-BAG\',backingNo:\'BN6\',lastAction:\'แบ็กกิ้ง\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'},{pno:\'ISO-N6-LABEL\',backingNo:\'BN6\',lastAction:\'ปริ้นท์ใบลาเบล\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'},{pno:\'ISO-N6-OTHER\',backingNo:\'BN6\',lastAction:\'สแกนพัสดุออกคลัง\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'}];const isolationAlready=isolationTotal.slice(0,5);const isolationRow={...truthRow,id:\'isolation-row\',expectedParcels:12,enteredParcels:5,pendingParcels:7};const savedProbeBranch=state.branch;state.branch=\'NE1\';const isoNE1=pnoOperationalBuildTruth(isolationRow,isolationTotal,isolationAlready);state.branch=\'NE6\';const isoNE6=pnoOperationalBuildTruth(isolationRow,isolationTotal,isolationAlready);state.branch=\'BPL\';const isoBPL=pnoOperationalBuildTruth(isolationRow,isolationTotal,isolationAlready);state.branch=savedProbeBranch;const originBadge=expectedParcelsBadge({...truthRow,id:\'origin-truth-row\',attendanceType:\'ต้นทาง\'}),truthCurrentProbe={entered:truthCurrent.entered,pending:truthCurrent.pending,correction:truthCurrent.correction,percent:Number(truthCurrent.percent.toFixed(2)),remaining:truthCurrent.remainingRows?.length||0,remainingActions:[...new Set((truthCurrent.remainingRows||[]).map(x=>x.lastAction))],verified:truthCurrent.verified,reason:truthCurrent.reason,alreadyBagDoubleCounted:truthCurrent.candidatePnos?.has(\'ALREADY-0\')||false},hubIsolationProbe={ne1:{entered:isoNE1.entered,pending:isoNE1.pending,correction:isoNE1.correction},ne6:{entered:isoNE6.entered,pending:isoNE6.pending,correction:isoNE6.correction},bpl:{entered:isoBPL.entered,pending:isoBPL.pending,correction:isoBPL.correction},canon:[pnoOperationalCanonicalHub(\'02 NE1_HUB-นครราชสีมา\'),pnoOperationalCanonicalHub(\'BPL_BHUB\'),pnoOperationalCanonicalHub(\'26 NAK_BHUB-นครราชสีมา\'),pnoOperationalCanonicalHub(\'EA1_HUB-พานทอง\')]};'+
  ...
  assert.deepEqual(result.truthCurrentProbe?.entered,354,`${label}: V25 operational entered should be 354/355`);
  assert.deepEqual(result.truthCurrentProbe?.pending,1,`${label}: V25 operational pending should be 1/355`);
  assert.deepEqual(result.truthCurrentProbe?.correction,101,`${label}: V25 correction should be 101 own-HUB Backing pieces not already counted`);
  assert.deepEqual(result.truthCurrentProbe?.percent,99.72,`${label}: V25 operational percent should be 99.72`);
  assert.deepEqual(result.truthCurrentProbe?.remaining,1,`${label}: V25 remaining should be exactly 1`);
  assert.deepEqual(result.truthCurrentProbe?.remainingActions,['รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว'],`${label}: V25 changed the real MS remaining status`);
  assert.equal(result.truthCurrentProbe?.verified,true,`${label}: V25 truth was not detail-verified`);
  assert.equal(result.truthCurrentProbe?.reason,'OWN_HUB_BACKING_CORRECTED',`${label}: V25 correction reason mismatch`);
  assert.equal(result.truthCurrentProbe?.alreadyBagDoubleCounted,false,`${label}: V25 double-counted a parcel already in MS already`);
