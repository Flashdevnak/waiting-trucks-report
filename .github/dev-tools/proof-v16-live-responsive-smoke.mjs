...
const SMOKE_VERSION = '20260908-03';
...
async function waitReady(cdp, sessionId, name) {
  for (let i = 0; i < 160; i += 1) {
    const state = await evaluate(cdp, sessionId, `({
      href: location.href,
      ready: document.readyState,
      v16: Boolean(window.__PROOF_V16_READY__),
      toolbar: Boolean(document.querySelector('.proof-toolbar-v16')),
      command: Boolean(document.getElementById('proof-command-center-v10'))
    })`);
    if (state?.href?.startsWith(`${ORIGIN}/proof.html`) && state.ready === 'complete' && state.v16 && state.toolbar && state.command) return;
    await sleep(100);
  }
  throw new Error(`${name}: Proof V16 not ready`);
}
...
const probe = `(() => {
  ...
  const result = {
    ...
    quickInside: Boolean(dayInput?.closest('label')?.contains(quick)),
    quickCount: quick?.querySelectorAll('button').length || 0,
    quickLabels: [...(quick?.querySelectorAll('button strong') || [])].map(e => e.textContent.trim()),
    quickButtons: [...(quick?.querySelectorAll('button') || [])].map(rect),
    commandFilterFlag: window.__PROOF_V16_COMMAND_FILTER_FIX__ || '',
    departedFilter: departedCard?.dataset?.proofV10Filter || '',
    extraFilter: extraCard?.dataset?.proofV10Filter || '',
    stateFilterValues,
    v16Asset: resources.some(name => name.includes('/${EXPECTED_ASSET}')),
    v17Asset: resources.some(name => name.includes('/proof-v17')),
    columnsDisplay: css(columns)?.display || '',
    cellLabelDisplay: css(cellLabel)?.display || '',
    statusBg: css(status)?.backgroundColor || '',
    tagBg: css(tag)?.backgroundColor || '',
    vehicleBg: css(vehicle)?.backgroundColor || '',
...
function common(name, result, width) {
  assert.equal(result.viewport, width, `${name}: viewport mismatch`);
  assert.ok(result.docWidth <= width + 1, `${name}: document overflow ${result.docWidth}/${width}`);
  assert.ok(result.bodyWidth <= width + 1, `${name}: body overflow ${result.bodyWidth}/${width}`);
  assert.equal(result.quickInside, true, `${name}: quick-date detached`);
  assert.equal(result.quickCount, 3, `${name}: quick-date count`);
  assert.deepEqual(result.quickLabels, ['เมื่อวาน', 'วันนี้', 'พรุ่งนี้'], `${name}: quick-date labels`);
  assert.ok(result.controlHeights.length >= 7, `${name}: toolbar controls missing`);
  assert.ok(result.controlHeights.every(h => Math.abs(h - 44) <= 1), `${name}: controls must be equal 44px: ${result.controlHeights}`);
  assert.equal(result.commandFilterFlag, 'departed-extra-v1', `${name}: command filter runtime flag missing`);
  assert.equal(result.departedFilter, 'departed', `${name}: ออกแล้ว command card is not mapped to departed`);
  assert.equal(result.extraFilter, 'extra', `${name}: รถเสริม command card is not mapped to extra`);
  assert.ok(result.stateFilterValues.includes('departed'), `${name}: departed state option missing`);
  assert.ok(result.stateFilterValues.includes('extra'), `${name}: extra state option missing`);
  assert.equal(result.v16Asset, true, `${name}: current V16 asset missing`);
  assert.equal(result.v17Asset, false, `${name}: V17 asset requested`);
  for (const box of [result.toolbar, result.search, result.hub, result.day]) assert.ok(box && box.x >= -1 && box.right <= width + 1, `${name}: toolbar box overflow ${JSON.stringify(box)}`);
  for (const value of [result.statusBg, result.tagBg, result.vehicleBg]) assert.notEqual(value, 'rgba(0, 0, 0, 0)', `${name}: badge contrast missing`);
  assert.equal(result.timeGridDisplay, 'grid', `${name}: Hero A time split missing`);
  assert.ok(result.heroWidth <= width + 1, `${name}: Hero A overflow`);
}

function viewport(name, result) {
  if (name === 'desktop') {
    assert.ok(Math.abs(result.search.y - result.hub.y) <= 2 && Math.abs(result.hub.y - result.day.y) <= 2, 'desktop: Search/HUB/Date not aligned');
    assert.ok(result.search.x < result.hub.x && result.hub.x < result.day.x, 'desktop: Search/HUB/Date order wrong');
    assert.equal(result.columnsDisplay, 'grid', 'desktop: header must be visible');
    assert.equal(result.cellLabelDisplay, 'none', 'desktop: duplicate labels must be hidden');
  } else if (name === 'tablet') {
    assert.equal(result.columnsDisplay, 'none', 'tablet: desktop header must hide');
    assert.notEqual(result.cellLabelDisplay, 'none', 'tablet: cell labels required');
  } else {
    assert.equal(result.columnsDisplay, 'none', 'mobile: desktop header must hide');
    assert.notEqual(result.cellLabelDisplay, 'none', 'mobile: cell labels required');
    assert.ok(result.quickButtons.every(box => box.width >= 70 && box.height >= 44), `mobile: quick-date touch targets too small ${JSON.stringify(result.quickButtons)}`);
  }
}
...
async function run() {
  ...
  try {
    ...
    for (const [name, width, height, mobile] of [
      ...
      await waitReady(cdp, sessionId, name);
      const result = await evaluate(cdp, sessionId, probe);
      common(name, result, width);
    ...
    console.log('PROOF_V16_COMMAND_FILTERS=PASS');
...
const hardTimer = setTimeout(() => {
  console.error(`PROOF_V16_SMOKE_HARD_TIMEOUT=${HARD_TIMEOUT_MS}`);
  process.exit(124);
}, HARD_TIMEOUT_MS);

run()
  .then(() => { clearTimeout(hardTimer); })
  .catch(error => {
