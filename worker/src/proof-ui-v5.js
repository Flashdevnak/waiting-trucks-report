...
function proofV6Runtime() {
  const start = () => {
    ...
    if (window.__PROOF_V7_READY__) return;
    window.__PROOF_V7_READY__ = true;
    ...
    const openSession = () => {
      ...
      document.getElementById('proof-session-hub').textContent = P.state.branch; document.getElementById('proof-session-har').value = ''; document.getElementById('proof-session-status').textContent = 'Session นี้ใช้ทั้งดูข้อมูล Proof และสร้าง PDF จาก MS'; document.getElementById('proof-session-dialog').showModal(); testSession(false);
    };
    ...
    const style = document.createElement('style'); style.id = 'proof-v7-style'; style.textContent = `
      ...
      @media(max-width:430px){.proof-session-panel{display:block}.proof-session-panel button{margin-top:8px;width:100%}.proof-ms-driver{grid-template-columns:1fr}.proof-ms-driver>div+div{border-left:0;border-top:1px solid #ddd}.proof-session-actions{grid-template-columns:1fr}.proof-editor-card,.proof-session-card{padding:10px}}
