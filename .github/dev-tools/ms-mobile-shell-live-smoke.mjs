...
async function probePnoModal(cdp,sessionId,width,label){
  const expression = '(async()=>{'+
    ...
    'try{'+
      ...
      'const truthRow={id:\'truth-row\',hub:\'NE1\',attendanceType:\'ปลายทาง\',proofId:\'TRUTH-BC\',pnoState:\'OK\',pnoEnabled:true,pnoSourceDay:\'2026-09-19\',expectedParcels:355,enteredParcels:239,pendingParcels:116};const truthCandidates=Array.from({length:113},(_,i)=>({pno:\'TRUTH-BAG-\'+i,backingNo:\'BAG-\'+Math.floor(i/40),lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\'}));const truthLinked=Array.from({length:3},(_,i)=>({pno:\'TRUTH-LINK-\'+i,backingNo:\'\',lastAction:\'รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว\',targetHub:i===2?\'20 NE6_HUB-บุรีรัมย์\':\'02 NE1_HUB-นครราชสีมา\'}));const truth=pnoOperationalBuildTruth(truthRow,[...truthCandidates,...truthLinked]),truthCurrent=pnoOperationalBuildTruth({...truthRow,enteredParcels:241,pendingParcels:114},[...truthCandidates,truthLinked[0]]),originBadge=expectedParcelsBadge({...truthRow,id:\'origin-truth-row\',attendanceType:\'ต้นทาง\'}),truthProbe={entered:truth.entered,pending:truth.pending,correction:truth.correction,percent:Number(truth.percent.toFixed(2)),remaining:truth.remainingRows?.length||0,remainingActions:[...new Set((truth.remainingRows||[]).map(x=>x.lastAction))],rawLinked:pnoV18ParcelAction(truthLinked[0])},truthCurrentProbe={entered:truthCurrent.entered,pending:truthCurrent.pending,correction:truthCurrent.correction,percent:Number(truthCurrent.percent.toFixed(2)),remaining:truthCurrent.remainingRows?.length||0};'+
