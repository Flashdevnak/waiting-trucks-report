...
async function probePnoModal(cdp,sessionId,width,label){
  const expression = '(async()=>{'+
    ...
    'try{'+
      ...
      'const truthRow={id:\'truth-row\',hub:\'NE1\',attendanceType:\'ปลายทาง\',proofId:\'TRUTH-BC\',pnoState:\'OK\',pnoEnabled:true,pnoSourceDay:\'2026-09-19\',pnoCanReport:false,expectedParcels:355,enteredParcels:239,pendingParcels:116};const truthCandidates=Array.from({length:113},(_,i)=>({pno:\'TRUTH-BAG-\'+i,backingNo:\'BAG-\'+Math.floor(i/40),lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\'}));const truthLinked=Array.from({length:3},(_,i)=>({pno:\'TRUTH-LINK-\'+i,backingNo:\'\',lastAction:\'รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว\',targetHub:i===2?\'20 NE6_HUB-บุรีรัมย์\':\'02 NE1_HUB-นครราชสีมา\'}));const truth=pnoOperationalBuildTruth(truthRow,[...truthCandidates,...truthLinked]),originBadge=expectedParcelsBadge({...truthRow,id:\'origin-truth-row\',attendanceType:\'ต้นทาง\'}),truthProbe={entered:truth.entered,pending:truth.pending,correction:truth.correction,percent:Number(truth.percent.toFixed(2)),remaining:truth.remainingRows?.length||0,remainingActions:[...new Set((truth.remainingRows||[]).map(x=>x.lastAction))],rawLinked:pnoV18ParcelAction(truthLinked[0])};const truthCurrentRow={...truthRow,id:\'truth-current-row\',proofId:\'TRUTH-CURRENT-BC\',pnoLineId:\'LINE-TRUTH\',pnoVanLineId:\'\',pnoStoreId:\'STORE-A\',pnoNextStoreId:\'STORE-B\',enteredParcels:241,pendingParcels:114};const pendingCurrent=Array.from({length:113},(_,i)=>({pno:\'CURRENT-BAG-\'+i,backingNo:\'\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'no_entry\'})).concat([{pno:\'CURRENT-LINK-0\',backingNo:\'\',lastAction:\'รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'no_entry\'}]);const totalCurrent=Array.from({length:113},(_,i)=>({pno:\'CURRENT-BAG-\'+i,backingNo:\'BAG-\'+Math.floor(i/40),lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'total\'})).concat([{pno:\'CURRENT-LINK-0\',backingNo:\'\',lastAction:\'รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'total\'},{pno:\'ALREADY-BAG-EXTRA\',backingNo:\'BAG-ALREADY\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'total\'}]);const visualBrowserPnoPage=browserPnoPage;browserPnoPage=async (_row,type,page)=>type===\'no_entry\'?{total:114,pageSize:200,pages:1,page,parcels:pendingCurrent}:type===\'total\'?{total:115,pageSize:200,pages:1,page,parcels:totalCurrent}:{total:0,pageSize:200,pages:1,page,parcels:[]};const truthCurrentKey=pnoOperationalKey(truthCurrentRow);pnoOperationalTruthCache.delete(truthCurrentKey);pnoOperationalEnteredRowsCache.delete(truthCurrentKey);const truthCurrent=await pnoOperationalResolve(truthCurrentRow);browserPnoPage=visualBrowserPnoPage;const truthCurrentProbe={entered:truthCurrent.entered,pending:truthCurrent.pending,correction:truthCurrent.correction,percent:Number(truthCurrent.percent.toFixed(2)),remaining:truthCurrent.remainingRows?.length||0,remainingActions:[...new Set((truthCurrent.remainingRows||[]).map(x=>x.lastAction))],usedTotalFallback:truthCurrent.usedTotalFallback,totalPagesRead:truthCurrent.totalPagesRead,unresolvedMetadata:truthCurrent.unresolvedMetadata,extraAlreadyCounted:truthCurrent.candidatePnos?.has(\'ALREADY-BAG-EXTRA\')||false};'+
      'return {ok:true,styleElement:Boolean(document.querySelector(\'#pno-approved-modal-v18-style\')),runtimeV2:(typeof pnoV18ViewCache!==\'undefined\'&&typeof pnoV18LoadBags===\'function\'),head,tableHead,desktop,mobile,bagTabStyle,bagCard,backingBadge,parcelMobileBagBox,parcelTableHeaders,bagTableHeaders,lineCopyButton,lineCopyText,clientFilterBar,parcelFilterCount,bagFilterCount,bagFilterLabels,bagSummaryText,truthProbe,truthCurrentProbe,originBadge,scrollInfo,dialog:dr?{width:dr.width,right:dr.right,x:dr.x,scrollWidth:dialog.scrollWidth,clientWidth:dialog.clientWidth}:null,viewport:innerWidth};'+
  ...
  assert.deepEqual(result.truthCurrentProbe?.entered,354,`${label}: live-style operational entered should be 354/355`);
  assert.deepEqual(result.truthCurrentProbe?.pending,1,`${label}: live-style operational pending should be 1/355`);
  assert.deepEqual(result.truthCurrentProbe?.correction,113,`${label}: live-style correction should remain 113`);
  assert.deepEqual(result.truthCurrentProbe?.percent,99.72,`${label}: live-style operational percent should be 99.72`);
  assert.deepEqual(result.truthCurrentProbe?.remaining,1,`${label}: live-style remaining should be 1`);
  assert.deepEqual(result.truthCurrentProbe?.remainingActions,['รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว'],`${label}: V23 remaining status must stay raw MS text`);
  assert.equal(result.truthCurrentProbe?.usedTotalFallback,true,`${label}: V23 did not enrich pending PNO from total metadata`);
  assert.equal(result.truthCurrentProbe?.totalPagesRead,1,`${label}: V23 should stop total fallback as soon as metadata is complete`);
  assert.equal(result.truthCurrentProbe?.unresolvedMetadata,0,`${label}: V23 left pending metadata unresolved in fixture`);
  assert.equal(result.truthCurrentProbe?.extraAlreadyCounted,false,`${label}: V23 double-counted a total-only already parcel`);
