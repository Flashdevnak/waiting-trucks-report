...
async function probePnoModal(cdp,sessionId,width,label){
  ...
  assert.deepEqual(result.truthCurrentProbe?.entered,354,`${label}: V24 operational entered should be 354/355`);
  assert.deepEqual(result.truthCurrentProbe?.pending,1,`${label}: V24 operational pending should be 1/355`);
  assert.deepEqual(result.truthCurrentProbe?.correction,113,`${label}: V24 correction should be 113 own-HUB Backing scans`);
  assert.deepEqual(result.truthCurrentProbe?.percent,99.72,`${label}: V24 operational percent should be 99.72`);
  assert.deepEqual(result.truthCurrentProbe?.remaining,1,`${label}: V24 remaining should be exactly 1`);
  assert.deepEqual(result.truthCurrentProbe?.remainingActions,['รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว'],`${label}: V24 changed the real MS remaining status`);
  assert.equal(result.truthCurrentProbe?.verified,true,`${label}: V24 truth was not detail-verified`);
  assert.equal(result.truthCurrentProbe?.reason,'OWN_HUB_BACKING_CORRECTED',`${label}: V24 correction reason mismatch`);
  assert.equal(result.truthCurrentProbe?.alreadyBagDoubleCounted,false,`${label}: V24 double-counted a parcel already in MS already`);
