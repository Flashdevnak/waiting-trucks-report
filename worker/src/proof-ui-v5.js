...
function proofV6Runtime() {
  const start = () => {
    ...
    const type = row => row?.lineTypeText || 'ยังไม่ระบุ';
    const vehicle = row => row?.plateTypeText || 'ยังไม่ระบุ';
    ...
    const card = row => {
      ...
      return `<article class="proof-ms-card ${Number(row.proofState) === 1 ? 'needs-action' : ''}">
        ...
        <header class="proof-ms-head"><div class="proof-ms-badges"><span class="proof-ms-dark">ต้นทาง</span><span class="proof-ms-vehicle">${P.esc(vehicle(row))}</span></div><h3>${P.esc(row.lineName || 'ยังไม่ระบุ')}</h3><p><b>${P.esc(barcode(row))}</b>   ${P.esc(plate)}</p></header>
    ...
    P.tableRow = row => {
      const plate = [row.plateTypeText, row.plateNumber].filter(Boolean).join('   ') || 'ยังไม่กำหนดทะเบียน';
      return `<tr><td class="proof-route"><strong>${P.esc(row.lineName || 'ยังไม่ระบุ')}</strong><small>${P.routeBadges(row)} ${P.esc(P.destinationLabel(row))}</small></td><td class="proof-car"><strong>${P.esc(plate)}</strong><small>${P.esc(mode(row))}</small></td><td class="proof-driver"><strong>${P.esc(row.driver || 'ยังไม่กำหนด')}</strong><small>${P.esc(row.driverPhone || 'ยังไม่ระบุ')}</small></td><td><strong>${P.esc(P.standbyText(row))}</strong><small>${P.esc(row.departureDate || '')}</small></td><td><strong>${P.esc(barcode(row))}</strong></td><td>${P.stateBadge(row)}${P.standbyBadge(row)}</td><td>${P.actionButtons(row)}</td></tr>`;
    };
    ...
    const openSession = () => {
      ...
      document.getElementById('proof-session-hub').textContent = P.state.branch; document.getElementById('proof-session-har').value = ''; const printHar=document.getElementById('proof-print-har'); if(printHar) printHar.value=''; const printHarName=document.getElementById('proof-print-har-file-name'); if(printHarName) printHarName.textContent='ยังไม่ได้เลือกไฟล์'; const printHarSave=document.getElementById('proof-print-har-save'); if(printHarSave) printHarSave.disabled=true; document.getElementById('proof-session-status').textContent = 'HAR ไฟล์เดียว ใช้ทั้งเส้นทาง MS และปริ้นบาร์โค้ด'; document.getElementById('proof-session-dialog').showModal(); testSession(false);
    };
