...
function renamed(fn, from, to) {
  return fn.toString().replace(`function ${from}`, `function ${to}`);
}
...
function isPhoneDesktopSiteLayoutReplacement(
...
function useMobileCardLayoutReplacement(
  viewportWidth = typeof window !== "undefined" ? window.innerWidth : 0,
  screenWidth = typeof screen !== "undefined" ? screen.width : 0,
  screenHeight = typeof screen !== "undefined" ? screen.height : 0,
) {
...
function renderRowsProgressivelyReplacement(rows) {
...
const responsive = [
  renamed(isPhoneDesktopSiteLayoutReplacement, "isPhoneDesktopSiteLayoutReplacement", "isPhoneDesktopSiteLayout"),
  renamed(useMobileCardLayoutReplacement, "useMobileCardLayoutReplacement", "useMobileCardLayout"),
  renamed(renderRowsProgressivelyReplacement, "renderRowsProgressivelyReplacement", "renderRowsProgressively"),
].join("\n\n");
s = replaceBetween(s, "function renderRowsProgressively(rows) {", "async function loadCompletedTodayRows()", responsive);

function completedTodayDatasetKeyReplacement(total = state.completedToday) {
  return `${state.branch}|${bangkokDateValue(new Date())}|${Number(total) || 0}`;
}
function completedTodayDatasetRowsReplacement() {
  return mergeLatest(state.archiveRows, state.currentRows).filter(isCompletedToday);
}
function completedTodayDatasetReadyReplacement() {
...
function shouldHydrateCompletedTodayRowsReplacement() {
  const expected = Number(state.completedToday) || 0;
...
async function loadCompletedTodayRowsReplacement(force = false) {
...
const completedHelpers = [
  renamed(completedTodayDatasetKeyReplacement, "completedTodayDatasetKeyReplacement", "completedTodayDatasetKey"),
  renamed(completedTodayDatasetRowsReplacement, "completedTodayDatasetRowsReplacement", "completedTodayDatasetRows"),
  renamed(completedTodayDatasetReadyReplacement, "completedTodayDatasetReadyReplacement", "completedTodayDatasetReady"),
  renamed(shouldHydrateCompletedTodayRowsReplacement, "shouldHydrateCompletedTodayRowsReplacement", "shouldHydrateCompletedTodayRows"),
  loadCompletedTodayRowsReplacement.toString().replace("loadCompletedTodayRowsReplacement", "loadCompletedTodayRows"),
].join("\n\n");
s = replaceBetween(s, "async function loadCompletedTodayRows()", "function renderFilterSummary(rows)", completedHelpers);
...
function operationIconReplacement(kind) {
  const paths = {
...
s = replaceBetween(s, "function operationIcon(kind) {", "function operationTimeline(stages, activeIndex)", renamed(operationIconReplacement, "operationIconReplacement", "operationIcon"));

function unloadCompletionCardReplacement(row) {
...
s = replaceBetween(s, "function unloadCompletionCard(row) {", "function renderOriginOperation(row)", renamed(unloadCompletionCardReplacement, "unloadCompletionCardReplacement", "unloadCompletionCard"));

function renderOriginOperationReplacement(row) {
...
s = replaceBetween(s, "function renderOriginOperation(row) {", "function renderDropOperation(row)", renamed(renderOriginOperationReplacement, "renderOriginOperationReplacement", "renderOriginOperation"));

function renderDropOperationReplacement(row) {
  const arrival = parseDate(row.actualArrivalAt);
  const departure = parseDate(row.actualDepartureAt);
  if (!arrival) return "";
...
s = replaceBetween(s, "function renderDropOperation(row) {", "function renderOperation(row)", renamed(renderDropOperationReplacement, "renderDropOperationReplacement", "renderDropOperation"));
...
.ms-page #filter-summary button:before{content:"";position:absolute;inset:11px auto 11px 0;width:3px;border-radius:0 4px 4px 0;background:var(--card-accent)}.ms-page #filter-summary button .operation-icon{width:21px;height:21px;padding:6px;border-radius:9px;background:#ffffffd9;color:var(--card-accent)}.ms-page #filter-summary button span{min-width:0;color:#49535a;font-size:12px;font-weight:750;line-height:1.3;text-align:left}.ms-page #filter-summary button strong{min-width:28px;color:#151a1e;font-size:27px;font-weight:900;line-height:1;text-align:right;font-variant-numeric:tabular-nums}.ms-page #filter-summary button:hover{border-color:color-mix(in srgb,var(--card-accent) 45%,#d9dfe2);box-shadow:0 6px 16px #1b2b3614}.ms-page #filter-summary button:focus-visible{outline:3px solid #151515;outline-offset:2px}.ms-page #filter-summary button.is-active{border-color:var(--card-accent);box-shadow:0 0 0 2px color-mix(in srgb,var(--card-accent) 18%,transparent),0 6px 16px #1b2b3614}
...
.ms-page .table-panel{background:#fff}.ms-page .table-scroll{max-width:100%;overflow-x:auto;overscroll-behavior-inline:contain}.ms-page .ms-table{width:100%;min-width:1400px;border-collapse:separate;border-spacing:0;background:#fff}.ms-page .ms-table thead th{background:#1f2428;color:#fff;text-align:center;vertical-align:middle;border-color:#32393e;font-size:12px;font-weight:850;letter-spacing:.01em}.ms-page .ms-table tbody tr,.ms-page .ms-table tbody tr:nth-child(even),.ms-page .ms-table tbody tr.is-unloading{background:#fff}.ms-page .ms-table tbody td{padding:14px 11px;border-color:#e6e9eb;background:#fff;vertical-align:middle}.ms-page .ms-table tbody tr:nth-child(even) td{background:#fcfdfd}.ms-page .ms-table tbody tr:hover td{background:#f4f7f8}
.ms-page .ms-table .col-route{width:24%}.ms-page .ms-table .col-meta{width:12%}.ms-page .ms-table .col-work{width:9%}.ms-page .ms-table .col-schedule{width:19%}.ms-page .ms-table .col-status{width:24%}.ms-page .ms-table .col-company{width:12%}.ms-page .route-summary{text-align:left}.ms-page .route-title{font-size:13px;line-height:1.45}.ms-page .route-code strong{font-size:14px}.ms-page .attendance-cell,.ms-page .schedule-stack.single,.ms-page .work-summary{text-align:center}.ms-page .type-badge{display:inline-flex;align-items:center;justify-content:center;min-width:82px;padding:7px 11px;border:1px solid transparent;border-radius:999px;font-size:12px;font-weight:850;line-height:1.15}.ms-page .type-badge.inbound{border-color:#e8c46b;background:var(--ms-destination-soft);color:#765000}.ms-page .type-badge.outbound{border-color:#c9bce9;background:var(--ms-origin-soft);color:#4f3a94}.ms-page .type-badge.drop{border-color:#afd3eb;background:var(--ms-drop-soft);color:#0f5f92}
