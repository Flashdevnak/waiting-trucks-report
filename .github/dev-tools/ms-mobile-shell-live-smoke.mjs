...
async function probePnoModal(cdp,sessionId,width,label){
  const expression = '(async()=>{'+
    ...
    'try{'+
      'const row={id:\'pno-smoke-row\',proofId:\'SMOKE-BC-001\',routeName:\'SMOKE ROUTE\',attendanceType:\'ปลายทาง\',hub:\'NE1\',pnoState:\'OK\',pnoEnabled:true,pnoSourceDay:\'2026-09-19\',pnoLineId:\'LINE-SMOKE\',pnoVanLineId:\'\',pnoStoreId:\'STORE-A\',pnoNextStoreId:\'STORE-B\',expectedParcels:60,enteredParcels:60,pendingParcels:0};'+
      ...
      'browserPnoPage=async (_row,type,page)=>({proofId:row.proofId,routeName:row.routeName,total:60,page,parcels:type===\'no_entry\'?[]:parcels});'+
      ...
      'const head=style(\'.pno-v18-head\'),tableHead=style(\'.pno-v18-table th\'),desktop=style(\'.pno-v18-desktop\'),mobile=style(\'.pno-v18-mobile\'),parcelTableHeaders=[...document.querySelectorAll(\'.pno-v18-desktop .pno-v18-table thead th\')].map(x=>x.textContent.trim()),parcelMobileBagBox=style(\'.pno-v18-mobile-bagbox\'),lineCopyButton=style(\'#copy-line-pending-parcels\'),clientFilterBar=style(\'#pno-v18-filterbar\'),parcelFilterCount=document.querySelectorAll(\'#pno-v18-filterbar select\').length;'+
      ...
      'const truthRow={id:\'truth-row\',hub:\'NE1\',attendanceType:\'ปลายทาง\',proofId:\'TRUTH-BC\',pnoState:\'OK\',pnoEnabled:true,pnoSourceDay:\'2026-09-19\',pnoCanReport:false,expectedParcels:355,enteredParcels:253,pendingParcels:102};const ownHubCurrent=Array.from({length:113},(_,i)=>({pno:\'OWN-BAG-\'+i,backingNo:\'BAG-\'+Math.floor(i/40),lastAction:i<96?\'สแกนเข้าคลัง\':i<112?\'แบ็กกิ้ง\':\'ปริ้นท์ใบลาเบล\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'total\'}));const alreadyOwnHub=ownHubCurrent.slice(0,12);const alreadyNonBag=Array.from({length:241},(_,i)=>({pno:\'ALREADY-NONBAG-\'+i,backingNo:\'\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'already\'}));const alreadyCurrent=[...alreadyNonBag,...alreadyOwnHub];const linkedCurrent={pno:\'CURRENT-LINK-0\',backingNo:\'\',lastAction:\'รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว\',targetHub:\'02 NE1_HUB-นครราชสีมา\',category:\'total\'};const pendingCurrent=[...ownHubCurrent.slice(12),linkedCurrent];const totalCurrent=[...alreadyNonBag,...ownHubCurrent,linkedCurrent];const visualBrowserPnoPage=browserPnoPage;browserPnoPage=async (_row,type,page)=>{const src=type===\'total\'?totalCurrent:type===\'no_entry\'?pendingCurrent:type===\'already\'?alreadyCurrent:[];const pageSize=200,pages=Math.max(1,Math.ceil(src.length/pageSize)),start=(Math.max(1,page)-1)*pageSize;return {total:src.length,pageSize,pages,page,parcels:src.slice(start,start+pageSize)}};const truthCurrentKey=pnoOperationalKey(truthRow);pnoOperationalTruthCache.delete(truthCurrentKey);pnoOperationalEnteredRowsCache.delete(truthCurrentKey);const truthCurrent=await pnoOperationalResolve(truthRow);browserPnoPage=visualBrowserPnoPage;const isolationTotal=[...Array.from({length:5},(_,i)=>({pno:\'ISO-A-\'+i,lastAction:\'ดำเนินการปกติ\',targetHub:\'02 NE1_HUB-นครราชสีมา\'})),{pno:\'ISO-N1-SCAN\',backingNo:\'BN1\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'02 NE1_HUB-นครราชสีมา\'},{pno:\'ISO-N1-BAG\',backingNo:\'BN1\',lastAction:\'แบ็กกิ้ง\',targetHub:\'02 NE1_HUB-นครราชสีมา\'},{pno:\'ISO-N1-LABEL\',backingNo:\'BN1\',lastAction:\'ปริ้นท์ใบลาเบล\',targetHub:\'02 NE1_HUB-นครราชสีมา\'},{pno:\'ISO-N6-SCAN\',backingNo:\'BN6\',lastAction:\'สแกนเข้าคลัง\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'},{pno:\'ISO-N6-BAG\',backingNo:\'BN6\',lastAction:\'แบ็กกิ้ง\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'},{pno:\'ISO-N6-LABEL\',backingNo:\'BN6\',lastAction:\'ปริ้นท์ใบลาเบล\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'},{pno:\'ISO-N6-OTHER\',backingNo:\'BN6\',lastAction:\'สแกนพัสดุออกคลัง\',targetHub:\'20 NE6_HUB-บุรีรัมย์\'}];const isolationPending=isolationTotal.slice(5);const isolationRow={...truthRow,id:\'isolation-row\',expectedParcels:12,enteredParcels:5,pendingParcels:7};const savedProbeBranch=state.branch;state.branch=\'NE1\';const isoNE1=pnoOperationalBuildTruth(isolationRow,isolationTotal,isolationPending);state.branch=\'NE6\';const isoNE6=pnoOperationalBuildTruth(isolationRow,isolationTotal,isolationPending);state.branch=\'BPL\';const isoBPL=pnoOperationalBuildTruth(isolationRow,isolationTotal,isolationPending);state.branch=savedProbeBranch;const originBadge=expectedParcelsBadge({...truthRow,id:\'origin-truth-row\',attendanceType:\'ต้นทาง\'}),truthCurrentProbe={entered:truthCurrent.entered,pending:truthCurrent.pending,correction:truthCurrent.correction,percent:Number(truthCurrent.percent.toFixed(2)),remaining:truthCurrent.remainingRows?.length||0,remainingActions:[...new Set((truthCurrent.remainingRows||[]).map(x=>x.lastAction))],verified:truthCurrent.verified,reason:truthCurrent.reason,nonPendingBagDoubleCounted:truthCurrent.candidatePnos?.has(\'OWN-BAG-0\')||false},hubIsolationProbe={ne1:{entered:isoNE1.entered,pending:isoNE1.pending,correction:isoNE1.correction},ne6:{entered:isoNE6.entered,pending:isoNE6.pending,correction:isoNE6.correction},bpl:{entered:isoBPL.entered,pending:isoBPL.pending,correction:isoBPL.correction},canon:[pnoOperationalCanonicalHub(\'02 NE1_HUB-นครราชสีมา\'),pnoOperationalCanonicalHub(\'BPL_BHUB\'),pnoOperationalCanonicalHub(\'26 NAK_BHUB-นครราชสีมา\'),pnoOperationalCanonicalHub(\'EA1_HUB-พานทอง\')]};'+
      'return {ok:true,styleElement:Boolean(document.querySelector(\'#pno-approved-modal-v18-style\')),runtimeV2:(typeof pnoV18ViewCache!==\'undefined\'&&typeof pnoV18LoadBags===\'function\'),head,tableHead,desktop,mobile,bagTabStyle,bagCard,backingBadge,parcelMobileBagBox,parcelTableHeaders,bagTableHeaders,lineCopyButton,lineCopyText,clientFilterBar,parcelFilterCount,bagFilterCount,bagFilterLabels,bagSummaryText,truthCurrentProbe,hubIsolationProbe,originBadge,scrollInfo,dialog:dr?{width:dr.width,right:dr.right,x:dr.x,scrollWidth:dialog.scrollWidth,clientWidth:dialog.clientWidth}:null,viewport:innerWidth};'+
  ...
  assert.equal(result.backingBadge,null,`${label}: literal Backing label should not be rendered`);
  assert.ok(result.lineCopyButton&&result.lineCopyButton.display!=='none',`${label}: LINE copy button is missing`);
  assert.ok(result.clientFilterBar&&result.clientFilterBar.display!=='none',`${label}: client filter bar is missing`);
  assert.equal(result.parcelFilterCount,2,`${label}: parcel filters should be status + latest action`);
  ...
  assert.deepEqual(result.truthCurrentProbe?.entered,354,`${label}: V28 operational entered should be 354/355`);
  assert.deepEqual(result.truthCurrentProbe?.pending,1,`${label}: V28 operational pending should be 1/355`);
  assert.deepEqual(result.truthCurrentProbe?.correction,101,`${label}: V28 correction should be 101 own-HUB Backing pieces still in provider pending membership`);
  assert.deepEqual(result.truthCurrentProbe?.percent,99.72,`${label}: V28 operational percent should be 99.72`);
  assert.deepEqual(result.truthCurrentProbe?.remaining,1,`${label}: V28 remaining should be exactly 1`);
  assert.deepEqual(result.truthCurrentProbe?.remainingActions,['รถที่เชื่อมโยงกับพัสดุถึงคลังแล้ว'],`${label}: V28 changed the real MS remaining action`);
  assert.equal(result.truthCurrentProbe?.verified,true,`${label}: V28 truth was not detail-verified`);
  assert.equal(result.truthCurrentProbe?.reason,'OWN_HUB_PENDING_BACKING_CORRECTED',`${label}: V28 correction reason mismatch`);
  assert.equal(result.truthCurrentProbe?.nonPendingBagDoubleCounted,false,`${label}: V28 double-counted a Backing parcel outside provider pending membership`);
  assert.deepEqual(result.hubIsolationProbe?.ne1,{entered:8,pending:4,correction:3},`${label}: NE1 user did not isolate NE1 bags`);
  assert.deepEqual(result.hubIsolationProbe?.ne6,{entered:9,pending:3,correction:4},`${label}: NE6 user did not isolate NE6 bags`);
  assert.deepEqual(result.hubIsolationProbe?.bpl,{entered:5,pending:7,correction:0},`${label}: BPL user incorrectly inherited another HUB bag correction`);
  assert.deepEqual(result.hubIsolationProbe?.canon,['NE1','BPL','NAK','EA1'],`${label}: HUB canonicalization mismatch`);
  assert.equal(result.originBadge,'',`${label}: origin route must not render inbound PNO metrics`);
  ...
  if(width>720) { assert.deepEqual(result.parcelTableHeaders?.slice(0,7),['#','PNO','สถานะ','ล่าสุด','จุดที่ระบุในข้อมูลพัสดุ','ชื่อสาขาต่อไป','เวลา'],`${label}: desktop parcel table columns are wrong`); assert.deepEqual(result.bagTableHeaders?.slice(0,8),['#','เลขถุงแบ็กกิ้ง','สถานะ','ล่าสุด','จำนวนพัสดุ','HUB ถัดไป','ชื่อสาขาต่อไป',''],`${label}: desktop bag table columns are wrong`); }
  if(width<=720){
