...
async function probePnoModal(cdp,sessionId,width,label){
  const expression = '(async()=>{'+
    ...
    'try{'+
      ...
      'return {ok:true,styleElement:Boolean(document.querySelector(\'#pno-approved-modal-v18-style\')),runtimeV2:(typeof pnoV18ViewCache!==\'undefined\'&&typeof pnoV18LoadBags===\'function\'),head,tableHead,desktop,mobile,bagTabStyle,bagCard,backingBadge,parcelMobileBagBox,parcelTableHeaders,bagTableHeaders,lineCopyButton,lineCopyText,clientFilterBar,parcelFilterCount,bagFilterCount,bagSummaryText,scrollInfo,dialog:dr?{width:dr.width,right:dr.right,x:dr.x,scrollWidth:dialog.scrollWidth,clientWidth:dialog.clientWidth}:null,viewport:innerWidth};'+
  ...
  assert.equal(result.bagFilterCount,3,`${label}: bag filters should be status + hub + branch`);
