(()=>{
  ...
  P.msInfo=(icon,label,value,sub='',cls='')=>`<div class='proof-ms-info ${cls}'><span class='proof-ms-info-icon'>${icon}</span><div><small>${P.esc(label)}</small><strong>${P.esc(value||'ยังไม่ระบุ')}</strong>${sub?`<em>${P.esc(sub)}</em>`:''}</div></div>`;
  P.groupRouteCard=row=>{
    ...
    return `<article class='proof-route-card proof-route-card-v4 ${Number(row.proofState)===1?'needs-action':''}'>
      <div class='proof-v4-head'>
        <div class='proof-v4-route'><strong>${P.esc(row.lineName||'ยังไม่ระบุ')}</strong><span class='proof-inline-badges'>${P.routeBadges(row)}</span></div>
  ...
  P.openEditor=(row,detail)=>{
    ...
    P.el('proof-editor-route').textContent=detail.lineName||row.lineName||'ยังไม่ระบุ';
  ...
  P.choosePlate=button=>{
    ...
    P.el('proof-editor-plate-selected').textContent=button.dataset.label||'ยังไม่ระบุ';
  ...
  P.chooseDriver=button=>{
    ...
    P.el('proof-editor-driver-selected').textContent=button.dataset.name||'ยังไม่ระบุ';P.el('proof-editor-driver-phone').textContent=button.dataset.phone||'ไม่มีเบอร์';
